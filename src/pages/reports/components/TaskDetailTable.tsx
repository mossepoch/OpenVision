import { dailyTaskDetails } from '../../../mocks/reportsData';
...
export default function TaskDetailTable({ searchQuery }: TaskDetailTableProps) {
  const filtered = dailyTaskDetails.filter(t => {
    ...
    <div className="overflow-x-auto">
          ...
          {filtered.map((task) => {
            ...
            return (
              ...
              >
                ...
                <td className="py-3 px-4">
                  <span className="text-[13px] text-gray-600">{task.station}</span>
                </td>
                <td className="py-3 px-4">
                  <div className="flex items-center gap-2">
                ...
                <td className="py-3 px-4">
                  <span className="text-[12px] text-gray-500">{task.startTime}</span>
                </td>
                <td className="py-3 px-4">
                  <span className="text-[13px] text-gray-600">{task.duration}</span>
                </td>
                <td className="py-3 px-4">
                  <div className="flex items-center gap-2">
      ...
      <div className="flex items-center justify-between py-4 px-4 border-t border-gray-100">
        ...
        <div className="flex items-center gap-1">
          <button className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-600 transition-colors cursor-pointer">
            <i className="ri-arrow-left-s-line text-[16px]"></i>
          </button>
          <button className="w-8 h-8 flex items-center justify-center rounded-lg bg-[#7c3aed] text-white font-medium text-[12px] cursor-pointer">
            1
          </button>
          <button className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-600 font-medium text-[12px] transition-colors cursor-pointer">
            2
          </button>
          <button className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-600 font-medium text-[12px] transition-colors cursor-pointer">
            3
